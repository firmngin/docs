...
flowchart LR
    A[Paid order] --> B{Device business mode}
    B -->|Service| C[Session starts on the device]
    C --> D[Session ends and the order closes]
    B -->|Vending| E[Device dispenses the paid units]
    E --> F[Receipt, no open session]
```
