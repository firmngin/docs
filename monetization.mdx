...
Every linked device runs in one business mode. **Service** sells session-based use of the device. **Vending** sells physical items that the device dispenses after payment, and it can also run an on-device kiosk checkout.

Business mode decides what happens after the order is confirmed. Prepaid and postpaid decide when the customer pays.
...
sequenceDiagram
    ...
    participant Device
    Client->>Firmngin: Complete the order
    alt Service mode
        ...
        Firmngin->>Client: Close the order
    else Vending mode
        Firmngin->>Device: Send the paid units to dispense
...
<CardGroup cols={2}>
  <Card title="Business Modes" icon="shuffle" href="/monetization/business-modes">
    Compare Service and Vending, and set the mode for a linked device.
  </Card>

  <Card title="Vending and kiosk" icon="box" href="/monetization/vending-mode">
    Multi-item carts, per-device stock, and kiosk checkout on the device screen.
  </Card>
</CardGroup>
