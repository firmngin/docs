...
| `Event.INIT` | `Init` | Initial device configuration (includes business mode `md`) |
| `Event.DISPENSE` | `Dispense` | Vending SKU array from `/dp` |
...
`Init.mode` comes from init key `md`: `1` = service, `2` = vending.
...
When business mode is vending, a paid prepaid order delivers SKUs as a JSON array on `/dp`:
