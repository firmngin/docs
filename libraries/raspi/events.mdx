...
On `connect()` the SDK subscribes to platform topics, publishes LWT online (`"1"`), syncs firmware info, and calls `request_init()` automatically.

On `disconnect()` or `stop()`, the SDK publishes LWT offline (`"0"`).

The Python SDK does **not** implement OTA firmware updates.
