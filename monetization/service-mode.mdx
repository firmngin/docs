...
sequenceDiagram
    ...
    opt Verification is configured
        Firmngin->>Device: Request PIN or readiness status
        loop Until verification passes
            Device->>Client: Show PIN or readiness state
            Client->>Firmngin: Submit verification
            Firmngin->>Client: Return the result
        end
    end
    alt Prepaid item
        ...
        Firmngin->>Device: Start the measured session
    end
