...
Order prerequisites can be checked before payment or after payment:

- **Before payment**: the customer can only continue to payment when the device is ready.
- **After payment**: the payment can complete first, then the device readiness check runs. If the check fails, the order needs follow-up handling.
...
sequenceDiagram
    ...
    Firmngin->>Merchant: Request PIN verification
    Merchant->>Client: Display the PIN
    Client->>Firmngin: Enter the PIN
    Firmngin->>Merchant: Confirm successful verification
    Merchant->>Client: Make the service available
```

If the PIN is incorrect or expires, the client must use the current PIN shown by the merchant before continuing.

### Prerequisite before payment flow

Choose **Before Payment** when the client should only pay after the merchant confirms that the service is ready.
...
sequenceDiagram
    ...
    Firmngin->>Merchant: Check service readiness
    Merchant->>Firmngin: Return readiness result
    alt Service is ready
        Firmngin->>Client: Show payment step
        Client->>Firmngin: Complete payment
        Firmngin->>Merchant: Confirm paid order
        Merchant->>Client: Provide the service
...
### Prerequisite after payment flow

Choose **After Payment** when payment should be completed before the final readiness check.
...
sequenceDiagram
    ...
    Merchant->>Firmngin: Return readiness result
    alt Service is ready
        Firmngin->>Client: Confirm service access
        Merchant->>Client: Provide the service
    else Service is not ready
        Firmngin->>Client: Show the required next step
    end
```

Use this option only when your operating process is prepared to assist a paid customer if the service is not ready.
