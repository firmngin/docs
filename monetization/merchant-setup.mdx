...
<Note>
  Use test/debug mode while configuring devices and flow rules. Use live mode only after the merchant is approved and ready to accept real payments.
</Note>
...
For most merchants, configure linked devices in this order:

1. Link the device to the merchant.
2. Open the linked device setup drawer.
3. Review the device overview and current service status.
