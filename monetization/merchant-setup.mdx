...
3. Choose the **Business Mode** for the device.
4. Configure **Map Entity** for customer-facing display.
5. Configure **Flow** for verification and stop mode.
6. Test the public page and checkout flow before going live.

## Business Mode

**Business Mode** decides how the device delivers what the customer paid for. Choose **Service** for session-based use of the device, or **Vending** for items the device dispenses after payment. Vending devices can also enable **Kiosk mode** so customers pay on the device screen.

Set business mode before preparing catalog items, because it affects which payment types the device can sell. See [Business Modes](/monetization/business-modes) for the full comparison.
