...
A price rule defines the base charge, billable quantity, unit price, and optional spending limit.
...
Tiered components use contiguous `[From, Until)` brackets: **From** is included and **Until** is excluded. The first bracket starts at 0, and the final bracket has no upper limit. The selected bracket contributes its configured fixed total once; the charge is not multiplied by usage.
