...
| **3** | Add template or device ID as a GitHub variable (if needed) |
...
## Step 3: Add deployment identifiers (optional)

Depending on your deployment mode, add GitHub **variables** under **Settings → Secrets and variables → Actions → Variables**:

| Variable | When to set |
| --- | --- |
| `FIRMNGIN_DEVICE_TEMPLATE_ID` | Fleet OTA via device template |
| `FIRMNGIN_DEVICE_SECRET_ID` | Single-device OTA (no template) |
...
2. Firmware source that builds with **PlatformIO** (`platformio.ini`) or **Arduino CLI** (`.ino` sketch).
...
Use variables for non-secret identifiers:

| Name | Used when |
| --- | --- |
| `FIRMNGIN_DEVICE_TEMPLATE_ID` | Fleet OTA via device template |
| `FIRMNGIN_DEVICE_SECRET_ID` | Single-device OTA |
