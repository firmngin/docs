...
| Action | How |
| --- | --- |
| View active tokens | See name, scopes, created date, and last used date |
...
For a sketch at the repo root instead, use `arduino-sketch: path_to_your_file.ino` and write `keys.h` to the repository root. See [Arduino CLI CI/CD](/firmware-ci-cd-arduino-cli) for both layouts.
...
| Version label | Set via workflow input, or auto-generated from git tag / commit |
| Supported binary | `.bin` from PlatformIO or Arduino CLI |
