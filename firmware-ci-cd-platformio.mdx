...
5. Locates `.pio/build/<env>/firmware.bin`.
...
Run the same commands locally that the action runs:
...
Fix compile errors locally before pushing — CI/CD will fail on the same errors.
...
| Error | Cause | Fix |
| --- | --- | --- |
| `Unable to detect PlatformIO environment` | No `default_envs` and ambiguous env list | Set `platformio-env` explicitly |
| `firmware.bin not found` | Custom output name or failed link step | Verify build output path; fix compile errors |
| `Firmware target does not match device template` | Board/model mismatch | Align template board fields with CI target |
| Build works locally, fails in CI | Missing files in git, wrong branch | Confirm all sources are committed; check `working-directory` |
